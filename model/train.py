...
def adjust_lr(opt, lr_pt_counter, es_pt_counter, best_loss, curr_loss, args):
    ...
    if best_loss < curr_loss:
        ...
        if lr_pt_counter > args['adj_lr_patience'] and args['adj_lr']:
            args['lr'] *= 0.1
            print('Changing learning rate to {} since no loss improvement over {} epochs.'.format(args['lr'],
                                                                                                  str(lr_pt_counter)))
...
def train(train_features, train_labels, val_features, val_labels, test_features, test_labels, network, config):
    ...
    if config['use_weights']:
        class_weights = class_weight.compute_class_weight('balanced', classes=np.unique(train_labels + 1),
                                                          y=train_labels + 1)
    ...
    for e in range(config['epochs']):
        ...
        if config['adj_lr'] or config['early_stopping']:
            best_loss, lr_pt_counter, es_pt_counter, improvement, stop = adjust_lr(opt, lr_pt_counter, es_pt_counter,
...
def predict(test_features, test_labels, network, config, log_date, log_timestamp):
    ...
    # test_preds.tofile('test_preds.csv', sep=',')
    # test_gt.tofile('test_gt.csv', sep=',')
    df = pd.DataFrame(test_gt.tolist(), test_preds.tolist())
    print(df.head())
